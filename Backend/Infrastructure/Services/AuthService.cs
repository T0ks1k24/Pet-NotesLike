...
public class AuthService : IAuthService
{
    ...
    public async Task<Result> LoginUserAsync(LoginDto loginDto)
    {
        if (string.IsNullOrWhiteSpace(loginDto.Email) || string.IsNullOrWhiteSpace(loginDto.Password))
            return Result.Failure("Email і пароль обов'язкові");

        var user = await _authRepository.GetUserByEmailAsync(loginDto.Email);
        if (user == null)
            return Result.Failure("Користувач не знайдений");

        var isPasswordValid = _passwordHasher.Verify(loginDto.Password, user.Password);
        if (!isPasswordValid)
